...
public class CyclicDialog extends Dialog {
    ...
    public CyclicDialog(CirSim sim) {
        ...
        for (int i = 0; i < sim.thermalSimulation.simTCEs.size(); i++) {
            GWT.log(sim.thermalSimulation.simTCEs.get(i).cvs.get(0).material.materialName);
            if (sim.thermalSimulation.simTCEs.get(i).cvs.get(0).material.magnetocaloric) {
                // TODO: This has to be modified, the flag will depend on the chosen cycle part type.
                componentIndices.add(i);
                magneticComponents.addItem(String.valueOf(sim.thermalSimulation.simTCEs.get(i).index) + " " + sim.thermalSimulation.simTCEs.get(i).name);
            }
        }
        ...
        for (int i = 0; i < sim.thermalSimulation.simTCEs.size(); i++) {
            availableComponents.addItem(String.valueOf(sim.thermalSimulation.simTCEs.get(i).index) + " " + sim.thermalSimulation.simTCEs.get(i).name);
        }
        ...
        applyButton.addClickHandler(new ClickHandler() {
            public void onClick(ClickEvent event) {
                ...
                sim.thermalSimulation.cycleParts.add(cyclePart);
                    ...
                    case PROPERTIES_CHANGE:
                        if (propsChangeDuration.isVisible() && !cyclePart.TCEs.contains(chosenComponent)) {
                            if (newRho.getValue() == 0 || newCp.getValue() == 0 || newK.getValue() == 0) {
                                ...
                                sim.thermalSimulation.cycleParts.remove(sim.thermalSimulation.cycleParts.size() - 1);
                                break;
                            }
...
// Add other duration keyup handlers for pressureFieldStrength, shearStressFieldStrength, etc.
            ...
            public void onChange(ChangeEvent event) {
                ...
                switch (addBox.getSelectedItemText()) {
                    case "Heat Transfer":
                        ...
                        cyclePart = new CyclePart(sim.thermalSimulation.cycleParts.size(), sim);
        ...
        magneticComponents.addChangeHandler(new ChangeHandler() {
            ...
            public void onChange(ChangeEvent event) {
                ...
                chosenComponent = sim.thermalSimulation.simTCEs.get(ci);
                ...
                for (int i = 0; i < sim.thermalSimulation.simTCEs.size(); i++) {
                    GWT.log(String.valueOf(sim.thermalSimulation.simTCEs.get(i).index) + " " + sim.thermalSimulation.simTCEs.get(i).name);
                }
        ...
        availableComponents.addChangeHandler(new ChangeHandler() {
            ...
            public void onChange(ChangeEvent event) {
                ...
                chosenComponent = sim.thermalSimulation.simTCEs.get(chosen);
    ...
    public void printCyclePart(CyclePart cp, HTML label) {
        if (sim.thermalSimulation.cycleParts.size() == 1) {
            if (label.getHTML().equals("")) {
                label.setHTML(label.getHTML() + "<b>Cyclic Operation:</b><br>");
                label.setHTML(label.getHTML() + "&emsp;<b>Cyclic</b> " + String.valueOf(sim.thermalSimulation.cyclic) + "<br>");
            } else {
                label.setHTML("");
            }
        }
    ...
    private void validateDurationInput(DoubleBox durationBox) {
        ...
        if (value != null && value < sim.thermalSimulation.dt) {
            durationBox.getElement().getStyle().setBorderColor(Color.red.getHexValue());
            durationBox.getElement().getStyle().setColor(Color.red.getHexValue());
        }
    }
