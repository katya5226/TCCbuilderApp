...
public class SwitchElm_SSM_01 extends SwitchElm {
    final double DEFINED_LENGTH = 0.005;

    public SwitchElm_SSM_01(int xx, int yy) {
        super(xx, yy);
        DEFINED_LENGTH_UNIT = CirSim.LengthUnit.MICROMETER_500;
    ...
    SwitchElm_SSM_01(int xx, int yy, boolean mm) {
        super(xx, yy, mm);
    }

    public SwitchElm_SSM_01(int xa, int ya, int xb, int yb, int f, StringTokenizer st) {
    ...
    public void initializeThermalControlElement() {
        ...
        constK = kOff;
        constRho = rhoOff;
        inputPower = 1600;  // estimated; may be incorrect
        crossArea = 0.0005;  // Unknown
    }
    ...
    public EditInfo getEditInfo(int n) {
        switch (n) {
            ...
            case 13:
                return new EditInfo("Response Time (s) - estimated", responseTime, false);
            case 14:
                return new EditInfo("Heat loss rate to the ambient (W/(m³K))", hTransv);
            case 15:
                return new EditInfo("Actuation input power (W) - estimated", inputPower, false);
            default:
                return null;
        }
    }
