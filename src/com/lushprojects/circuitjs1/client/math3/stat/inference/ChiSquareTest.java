 ...
 */
package com.lushprojects.circuitjs1.client.math3.stat.inference;

import com.lushprojects.circuitjs1.client.math3.distribution.ChiSquaredDistribution;
import com.lushprojects.circuitjs1.client.math3.exception.DimensionMismatchException;
import com.lushprojects.circuitjs1.client.math3.exception.MaxCountExceededException;
import com.lushprojects.circuitjs1.client.math3.exception.NotPositiveException;
import com.lushprojects.circuitjs1.client.math3.exception.NotStrictlyPositiveException;
import com.lushprojects.circuitjs1.client.math3.exception.NullArgumentException;
import com.lushprojects.circuitjs1.client.math3.exception.OutOfRangeException;
import com.lushprojects.circuitjs1.client.math3.exception.ZeroException;
import com.lushprojects.circuitjs1.client.math3.exception.util.LocalizedFormats;
import com.lushprojects.circuitjs1.client.math3.util.MathArrays;
...
public class ChiSquareTest {
	 ...
	 * Chi-Square statistic</a> comparing <code>observed</code> and
	 * <code>expected</code> frequency counts.
	 * 
	 * This statistic can be used to perform a Chi-Square test evaluating the
	 * null hypothesis that the observed counts follow the expected
	 * distribution.
	 * 
	 ...
	 * <code>IllegalArgumentException</code> is thrown. <strong>Note:
	 * </strong>This implementation rescales the <code>expected</code> array if
	 * necessary to ensure that the sum of the expected and observed counts are
	 * equal.
	 *
	 * @param observed
	 ...
	 *             if <code>expected</code> has entries that are not strictly
	 *             positive
	 ...
	 * frequency counts to those in the <code>expected</code> array. The number
	 * returned is the smallest significance level at which one can reject the
	 ...
	 * Performs a <a href=
	 * "http://www.itl.nist.gov/div898/handbook/eda/section3/eda35f.htm">
	 * Chi-square goodness of fit test</a> evaluating the null hypothesis that
	 * the observed counts conform to the frequency distribution described by
	 * the expected counts, with significance level <code>alpha</code>. Returns
	 * true iff the null hypothesis can be rejected with 100 * (1 - alpha)
	 * percent confidence. <strong>Example:</strong><br>
	 ...
	 * chi-square test of independence</a> based on the input
	 * <code>counts</code> array, viewed as a two-way table. The rows of the
	 * 2-way table are <code>count[0], ... , count[count.length - 1] </code>
	 * <strong>Preconditions</strong>:
	 * <ul>
	 * <li>All counts must be &ge; 0.</li>
	 * <li>The count array must be rectangular (i.e. all count[i] subarrays must
	 * have the same length).</li>
	 * <li>The 2-way table represented by <code>counts</code> must have at least
	 * 2 columns and at least 2 rows.</li>
	 * </ul>
	 * If any of the preconditions are not met, an
	 * <code>IllegalArgumentException</code> is thrown.
	 *
	 ...
	 * Performs a
	 * <a href="http://www.itl.nist.gov/div898/handbook/prc/section4/prc45.htm">
	 * chi-square test of independence</a> evaluating the null hypothesis that
	 * the classifications represented by the counts in the columns of the input
	 * 2-way table are independent of the rows, with significance level
	 * <code>alpha</code>. Returns true iff the null hypothesis can be rejected
	 * with 100 * (1 - alpha) percent confidence. The rows of the 2-way table
	 * are <code>count[0], ... , count[count.length - 1] </code>
	 * <strong>Example:</strong><br>
	 * To test the null hypothesis that the counts in
	 * <code>count[0], ... , count[count.length - 1] </code> all correspond to
	 * the same underlying probability distribution at the 99% level, use
	 * <code>chiSquareTest(counts, 0.01)</code> <strong>Preconditions</strong>:
	 * <ul>
	 * <li>All counts must be &ge; 0.</li>
	 * <li>The count array must be rectangular (i.e. all count[i] subarrays must
	 * have the same length).</li>
	 ...
	 * in <code>observed1</code> and <code>observed2</code>. The sums of
	 * frequency counts in the two samples are not required to be the same. The
	 * formula used to compute the test statistic is
	 * </p>
	 * <code>
	 ...
	 *             <code>observed2</code> are zero, or if the count at some
	 *             index is zero for both arrays
	 * @since 1.2
	 */
	 ...
	 * test evaluates the null hypothesis that the two lists of observed counts
	 * conform to the same frequency distribution, with significance level
	 * <code>alpha</code>. Returns true iff the null hypothesis can be rejected
	 * with 100 * (1 - alpha) percent confidence.
	 * </p>
	 * <p>
	 * See {@link #chiSquareDataSetsComparison(long[], long[])} for details on
	 * the formula used to compute the Chisquare statistic used in the test. The
	 * degrees of of freedom used to perform the test is one less than the
	 * common length of the input observed count arrays.
	 * </p>
	 * <strong>Preconditions</strong>:
	 * <ul>
	 * <li>Observed counts must be non-negative.</li>
	 ...
	 * @throws ZeroException
	 *             if either all counts of <code>observed1</code> or
	 *             <code>observed2</code> are zero, or if the count at the same
	 *             index is zero for both arrays
	 * @throws OutOfRangeException
	 *             if <code>alpha</code> is not in the range (0, 0.5]
	 * @throws MaxCountExceededException
